...
const userAgentApp =
  appConfig.clientID &&
  new UserAgentApplication(appConfig.clientID, null, () => null);
  ...
  public handleAuth = async () => {
    if (appConfig.clientID === "") {
      ...
      if (this.state.accessToken !== null) {
        ...
        console.warn(
          "AAD Client ID has not been configured. If you are currently in production mode, see the 'deploy' documentation for details on how to fix this."
        );
        this.setState({ accessToken: "" });
      }
    } else if (userAgentApp === "") {
      // normal or 'production' auth flow
      ...
      try {
        if (this.state.accessToken !== null) {
          ...
          if (userAgentApp) {
            ...
            accessToken = await userAgentApp.acquireTokenSilent(
              graphScopes,
